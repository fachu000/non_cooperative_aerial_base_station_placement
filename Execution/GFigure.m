classdef GFigure
	...
	properties
		...
		v_zlim = [];       % 1 x 2 vectors with the limits for the z-axis
		...
		v_caxis =[];       % Color axis. This is the argument to the caxis 
		                   ...
		                   % max] is applied. 
		b_colorbar = 0;    % if ~=0, adds a color bar to one side.
	...
	methods(Access=private)
		...
		function setColors(F)
			
			chldv=get(gca,'Children');
			for kr=size(F.m_Y,1):-1:1
				get(chldv(kr),'XData');
				try
					if isa( chldv(kr) ,'matlab.graphics.chart.primitive.Line')||...
							isa( chldv(kr) ,'matlab.graphics.chart.primitive.Stem')
						set(chldv(kr),'Color',F.m_colorset(mod(kr-1,min(F.colorPeriod,size(F.m_colorset,1)))+1,:));
					end
				catch
...
% 				try
% 					if isa( chldv(length(chldv)-kr+1) ,'matlab.graphics.chart.primitive.Line')||...
% 							isa( chldv(length(chldv)-kr+1) ,'matlab.graphics.chart.primitive.Stem')
